...
final3[Time==2022,by=.(agest),sum(pop)]

final3[Time==2022,sum(pop)]
# 203,080,756
final3[Time==2022,by=.(edu,agest),sum(pop)] %>% spread(edu,V1)
#clean the path_scen
...
# for(ireg in regions) {
  ...
  for (iper in seq(initime,fintime-ts,ts))
    ...
    final.temp = copy(final3)[Time == iper&region%in%ireg]# chose an many regions you want
  ...
  final3[copy(birthstot),on=id.cols,`:=`(pop=i.pop)] #pop1 will be updated later

  #Update the final with pop1, births, mig, edu transition
  final3[copy(final.temp),on=id.cols,
  ...
  final3[final.temp.end,on=id.cols,`:=`(pop = i.pop)]
...
## basic checks
    final3[agest!=-5,by=.(Time),sum(pop)]
    final3[pop<0,by=.(Time),sum(pop)]
#   #births
    final3[,by=.(Time),sum(births)]
    TFRcheck<-final3[agest%in% 15:45,by=.(Time,agest),.(births=sum(births))]
    TFRcheckw <- final3[agest%in% 15:45 & sex =="f",by=.(Time,agest),.(popf=sum(pop))]
    TFRcheck %<>% 
...
#   #deaths
    final3[,by=.(Time),sum(deaths)]#abs deaths
    final3[,by=.(Time),1000*sum(deaths)/5/sum(pop)]#CMR
#   migration
    final3[,by=.(Time),sum(idom)]-final3[,by=.(Time),sum(odom)]
