...

{
  ...
  ages = seq(-5,90,by=5)# -5 for newborns "alive" at the beginning of the time interval 
  nage = length(ages) #
  agecodes = ages/5
  ...
  #input data [ignore]
  ntrans = 8 #sx, asfr,srb, edu, inmigdom, outmigdom, immigration, emigration
  input.vars <-  c("pop",#population in '000
                   "sx", # survival ratio
                   "asfr", #births per 1000 years of exposure
                   "eaprlm", #edu attainment progression ratio... 
...
if(iscen!="baseline"){
  ...
  for(i in "asfr"){ #asfrdt
    xxxdt <- expand.grid(region = regions,
                         Time=pers[-length(pers)],sex=sexnames[1],edu=educodes,agest=fertages,stringsAsFactors=F)
    setDT(xxxdt)
    xxdtname <- paste(i,"dt",sep="")
    assign(xxdtname,xxxdt);rm(xxxdt)
  }
